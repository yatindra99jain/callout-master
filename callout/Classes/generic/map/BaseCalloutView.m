...
- (void)prepareOffset
{
    ...
    NSLog(@"frame last %@", NSStringFromCGRect(self.parentAnnotationView.frame));
...
- (void)adjustMapRegionIfNeeded
{
    NSLog(@"frame one %@", NSStringFromCGRect(self.frame));
    
    NSLog(@"frame two %@", NSStringFromCGPoint(self.bounds.origin));
    
    NSLog(@"frame three %@", NSStringFromCGSize(self.bounds.size));
    
    
    NSLog(@"x possition  %f", self.frame.origin.x);
    
    NSLog(@"y possition  %f", self.frame.origin.y - self.bounds.size.height);
